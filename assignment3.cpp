/***
 ...
 Name: Pegler-Gordon, Maya

 Collaborators: Bahar, Talia; Louis, Maddie
...
vector<GLfloat> build_cube()
{
    vector<GLfloat> vertices{
        ...
        -0.5, -0.5, +0.0,
        +0.5, -0.5, +0.0};
    vector<GLfloat> frontFace = mat_mult(translation_matrix(0.0, 0.0, 0.5), to_homogeneous_coord(init_plane()));           //A1
    vector<GLfloat> backFace = mat_mult(translation_matrix(0, 0, -0.5), mat_mult(rotation_matrix_y(180), init_plane()));   //A2
    vector<GLfloat> rightFace = mat_mult(translation_matrix(0.5, 0, 0), mat_mult(rotation_matrix_y(90), init_plane()));    //A3
    vector<GLfloat> leftFace = mat_mult(translation_matrix(-0.5, 0, 0), mat_mult(rotation_matrix_y(-90), init_plane()));   //A4
    vector<GLfloat> topFace = mat_mult(translation_matrix(0, 0.5, 0), mat_mult(rotation_matrix_x(-90), init_plane()));     //A5
    vector<GLfloat> bottomFace = mat_mult(translation_matrix(0, -0.5, 0), mat_mult(rotation_matrix_x(-90), init_plane())); //A6
    vector<GLfloat> result;
    // result = init_plane();
    result.reserve(frontFace.size() * 6);
    result.reserve(frontFace.size());

    result.insert(result.end(), frontFace.begin(), frontFace.end());
    result.insert(result.end(), backFace.begin(), backFace.end());
    result.insert(result.end(), rightFace.begin(), rightFace.end());
    result.insert(result.end(), leftFace.begin(), leftFace.end());
    result.insert(result.end(), topFace.begin(), topFace.end());
    result.insert(result.end(), bottomFace.begin(), bottomFace.end());

    // TODO: Creates a unit cube by transforming a set of planes
    for (int i = 0; i < frontFace.size(); i++)
    {
        std::cout << frontFace.at(i) << ' ';
        if ((i + 1) % 4 == 0 && i != 0)
        {
            std::cout << '\n';
        }
